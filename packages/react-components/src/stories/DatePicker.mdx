...
A date picker combines an input field, a button and a calendar. The user can enter a value by keyboard (by typing or incrementing each date segment's value individually), or by selecting a date on the calendar.
...
The date format is set automatically based on the user's browser locale.
