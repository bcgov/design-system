...
<InlineAlert
  ...
  description="It is based on the RA Switch component. Consult the React Aria docs to understand its full API."
  buttons={[
    <Link
      isButton
      size="small"
...
<Canvas of={SwitchStories.SwitchTemplate} />
...
Use the `description` slot to display additional explanatory text under the switch:

<Canvas of={SwitchStories.SwitchWithDescription} />
...
Use `isInvalid` when a switch is in an invalid state. An error message (populated via `errorMessage`) is displayed below the switch:

<Canvas of={SwitchStories.ErrorSwitch} />
